...
const Notes = () => {
  ...
  const fetchNotes = useCallback(async () => {
    ...
    try { const data = await getUserNotes(); setNotes(Array.isArray(data) ? data : []); }
    catch (e) { setError(e.response?.data?.error || e.message || "Failed to load notes."); setNotes([]); }
  ...
  const openDetail = async (note) => {
    setSelected(note); setDetail(null); setDetailLoading(true);
    try { const full = await getNoteById(note._id || note.id); setDetail(full); }
  ...
  const handleDownload = async (note) => {
    ...
    if (!id) return;
    setPdfBusy(true);
    try { await downloadNotePdf(id, note?.topic || detail?.topic); }
    ...
    return (
      <Container className="py-14 sm:py-20">
        <div className="flex flex-wrap items-center gap-3">
          <Button variant="outline" onClick={() => { setSelected(null); setDetail(null); }}><FiArrowLeft aria-hidden="true" /> Back to notes</Button>
          <Button variant="accent" onClick={() => handleDownload(display)} disabled={pdfBusy}><FiDownload aria-hidden="true" /> {pdfBusy ? "Preparing…" : "Download PDF"}</Button>
        </div>
        ...
        {detailLoading ? (
          <Panel className="mt-6 flex items-center justify-center py-16" padding="snug">
            <span className="inline-flex items-center gap-3 text-ink-3"><span className="h-5 w-5 animate-spin rounded-full border-2 border-line border-t-brand" aria-hidden="true" />Loading notes…</span>
          </Panel>
        ) : display.error ? (
          ...
          <Panel className="mt-6 overflow-hidden" padding="snug">
            ...
            <div className="mt-4 flex flex-wrap gap-3">
              <Button variant="accent" onClick={() => handleDownload(display)} disabled={pdfBusy}><FiDownload aria-hidden="true" /> Download PDF</Button>
  ...
  return (
    <Container className="py-14 sm:py-20">
      <PageHeader title="Notes" standfirst="Every set of notes ExaminAI has generated for you, newest first. Search across titles and subjects, or narrow to one paper." actions={<Button variant="accent" onClick={() => setBriefOpen(true)}>Generate new notes</Button>} />
      <Panel padding="snug" className="mt-10">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
          ...
          <div className="sm:w-52"><label htmlFor="note-sort" className="sr-only">Sort notes</label><Select id="note-sort" value={sort} onChange={(e) => setSort(e.target.value)}>{Object.entries(sorts).map(([value, option]) => (<option key={value} value={value}>{option.label}</option>))}</Select></div>
        </div>
        <div className="mt-4 flex flex-wrap gap-2 border-t border-line pt-4">
          {subjects.map((name) => (<Chip key={name} as="button" type="button" active={subject === name} aria-pressed={subject === name} onClick={() => setSubject(name)}>{name}</Chip>))}
        </div>
      </Panel>
      {loading ? (
        <Panel className="mt-8 flex items-center justify-center py-16" padding="snug"><span className="inline-flex items-center gap-3 text-ink-3"><span className="h-5 w-5 animate-spin rounded-full border-2 border-line border-t-brand" aria-hidden="true" />Loading your notes…</span></Panel>
      ) : error ? (
        <Panel className="mt-8 border-brand/20" padding="snug"><p className="inline-flex items-center gap-2 font-medium text-brand"><FiAlertCircle aria-hidden="true" /> {error}</p><div className="mt-4"><Button variant="outline" onClick={fetchNotes}>Retry</Button></div></Panel>
      ) : (
        <>
          <p aria-live="polite" className="mt-8 text-fine text-ink-3">{notes.length === 0 ? "No notes yet" : filtered.length === notes.length ? `${notes.length} sets of notes` : `${filtered.length} of ${notes.length} sets of notes`}</p>
          {filtered.length === 0 ? (
            <div className="mt-4"><EmptyState title={filtersApplied ? "Nothing matched that" : notes.length === 0 ? "No notes generated yet" : "Nothing matched"} action={filtersApplied ? <Button variant="outline" onClick={() => { setQuery(""); setSubject(ALL); }}>Clear search and filters</Button> : <Button variant="accent" onClick={() => setBriefOpen(true)}>Generate your first notes</Button>}>{filtersApplied ? "Try a shorter search, or pick a different subject." : "Paste a chapter or upload your slides, and the notes will appear here."}</EmptyState></div>
          ) : (
            <ul className="mt-4 space-y-4">{filtered.map((note) => (<li key={note._id || note.id}><NoteCard note={note} onOpen={openDetail} onDownload={handleDownload} pdfBusy={pdfBusy} /></li>))}</ul>
          )}
        </>
      )}
...
function NoteCard({ note, onOpen, onDownload, pdfBusy }) {
  ...
  return (
    <Panel as="article" padding="snug" className="transition-colors hover:border-line-firm">
      <MarginRail screenReaderLabel="Generated on" locator={created ? <time dateTime={note.createdAt} title={fullDate.format(created)}>{railDate.format(created)}</time> : null}>
        ...
        <div className="mt-4 flex flex-wrap items-center gap-3">
          ...
          <span className="flex gap-2"><Button variant="outline" onClick={() => onOpen(note)}><FiEye aria-hidden="true" /> View</Button><Button variant="accent" onClick={() => onDownload(note)} disabled={pdfBusy}><FiDownload aria-hidden="true" /> PDF</Button></span>
        </div>
      </MarginRail>
    </Panel>
  );
